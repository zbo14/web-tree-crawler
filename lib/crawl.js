...
const crawl = async (url, opts = {}) => {
  ...
  opts.verbose && console.log(`Crawling "${hostname}" for <= ${timeLimit} seconds\n`)
  ...
  while (!done && toVisit.length) {
    ...
    const promises = slice.map(async url => {
      try {
        ...
        opts.verbose && console.log(`Successfully visited "${url}"`)
      } catch (err) {
        opts.verbose && console.error(err.message)
      }
